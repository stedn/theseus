...
We first probed the effect of filament recycling on the network's passive response by imposing an external force on our simulated network in the absence of recycling.   As shown in Figure \ref{fig:passive_ex} part a), by applying a force at the boundary of a patch of network, we induced a rightward deformation of the network.  On short timescales the networked responded mostly elastically by approaching a fixed strain $\gamma_0$, similar to that predicted by \cite{theo_hlm}.  On times longer than $\tau_c$, we found that the network approached a steady linear strain, characterized by an effective viscosity, $\eta = \sigma/$.  This 

As the network continues to dilate viscously, the connectivity of the network decreases.  As network connectivity decreases, the strain rate of the network increases beyond the .  Eventually connected patches become isolated and the network tears, resulting in the highly heterogeneous network structure shown in panel c.  
...
The ratio of $G_0$ to $\eta_c$ is a general indicator of the transition timescale from elastic to viscous behavior in a viscoelastic material.  Using the approximation of the equation from \cite{theo_hlm}, $G_0 \approx \mu/l_c$, we predict a crossover time, $\tau_c \approx L^2\xi/l_c\mu$ 
...
As shown in Figure \ref{fig:passive}, we find that the crossover time, $\tau_c$ is in general agreement with our prediction.
...
Although this simplification still lacks the complexity involved in internal stress generation, this investigation presented us with an understanding of
