...
In long $\tau_r\gg\tau_c$ limit, this simplifies to $\eta_r\approx\eta_c$, while in the short $\tau_r\ll\tau_c$ limit, this simplifies to $\eta_r\sim\tau_r/\tau_c$, which matches qualitatively with our estimate as found in Figure \ref{fig:passive_rec}d for a large range of parameters.  This model presents a simple quantitative description of our simulation data.
...
Similar to equation \ref{eqn:simple_eta}, we can introduce an approximating equation to describe the dependence of the steady state stress on the filament recycling rate.  
...
In this case, in long $\tau_r\gg\tau_a$ limit, this simplifies to $\sigma_{ss}\sim(\tau_a/\tau_r)^n$, while in the short $\tau_r\ll\tau_a$ limit, this simplifies to $\sigma_{ss}\sim\tau_r/\tau_a$.  It isn't entirely clear what sets the scaling $n$ and the scaling does not appear to be consistent across all simulation setups as is made clear by Figure \ref{fig:active_rec}d.  However, this model still presents a qualitatively correct description of steady state stress in our simulation data.
...
To summarize the predicted behaviors we introduce Figure \ref{fig:flow_theo}. {\em need more explanation here}.
