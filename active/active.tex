...
Previous theoretical and experimental studies identified a set of minimal conditions for contraction of disordered networks:  The first is asymmetric compliance, which is an intrinsic property of actin filaments. The second is dispersion in motor speeds (or force?), such that motors induce compressive or extensional stress along segments of filaments between motor attachment points.  To test if our minimal implementation of these two requirements (see above) was sufficient, we simulated active networks that were unconstrained by external attachments.
...
{\em Need to describe this as a characteristic sequence with three phases:  rapid initial contraction, stall against compressive resistance, then filament sorting.Which parts of this sequence have been described elsewhere, either in experiments or models?  Need to cite appropriate references?The main goal of this section is to demonstrate that your model reproduces the mode of contractility already described by Lenz, Gardel, Murrell, etc.  So tick off the key points and relate each to the previous work: Whole network and individual filaments strain at the same ratecontraction requires the asymmetric compliance  and it requires dispersion of motor activity.Possible points of discussion: Your model identifies a limit to contraction when the resistance to compression becomes sufficiently high.  In Murrell and Gardel, this is relieved by buckling and snapping of filaments. No recycling necessary.  Is their result general? No, for two reasons:  (1) because they were using large minifilaments that are expected to induce significant force.  The  smaller minifilaments in non-muscle cells might not produce enough force to do the job. (2) their networks were not reinforced by additional cross linkers. Cross-linking and bundling of filaments would protect against breakage?}
...
{\em reorganize and shorten these next tow paragraphs around  a few key points:Describe the two main effects of recycling (a) prevents catastrophic tearing (that you should have already talked about in the last figure) and (b) modulates the level of steady state stress.  Emphasize that the origins of these two effects are essentially the same as for the passive case.In the latter case (modulation of steady state stress), you then want to emphasize the bimodal nature of the dependence and explain how this depends on the relationship between the recycling time, and the time course of buildup and dissipation of total stress in the absence of recycling. }
