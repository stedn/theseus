...
We first probed the effect of filament recycling on the network's passive response by imposing an external force on our simulated network in the absence of recoiling.   As shown in Figure \ref{fig:passive} part a), by applying a force at the boundary of a patch of network, we induced a net strain.  On short timescales the networked responded mostly elastically by approaching a fixed strain $\gamma_0$, similar to that predicted by \cite{theo_hlm}.  On times longer than $\tau_c$, we found that the network approached a steady linear strain rate, characterized by an effective viscosity, $\eta$.  This 

As the network continues to dilate viscously, the connectivity of the network decreases.  Eventually connected patches become isolated and the network tears, resulting in the highly heterogeneous network structure shown in panel c.  While this 
...
Across the parameter space of importance to us, the cell spends most of it's time undergoing viscous deformation with a nearly constant effective viscosity.  We were able to develop a simple theoretical description that captured the observed dependence.

\begin{equation}
\label{eqn:eff_vic}
\eta_c = \xi\left (\frac{L}{l_c}-1\right )^2
\end{equation}
...
\caption{\label{fig:passive}  Networks undergo three stages of deformation under extension in the presence of cross-link slip alone.  \textbf{a)} Example simulations of a $4\times10\: \mu m$ network deforming under an applied extensional stress of 0.005 $nN/\mu m$. Network parameters: $L=1\: \mu m$, $l_c=0.3\: \mu m$, $\xi=100\: nN\cdot s$. \textbf{b)} Stress and velocity profiles for the example network at t=88s.  Note that the stress is nearly constant and the velocity is nearly linear as predicted for a viscous fluid under extension.  \textbf{c)} Trace of the stress and strain buildup in an extending network.  The network begins to behave purely viscously at $\tau_c \approx 4 s$ as indicated by the vertical dotted line. The network deforms like a material with effective viscosity $\eta_c$ as indicated by the slope of the dashed line. (inset) At long times, the strain accelerates as the network undergoes strain thinning and eventually tears. }
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=\hsize]{figures/figure3b}
\caption{\label{fig:passive_form} Network architecture sets the rate and timescales of deformation. \textbf{a)} The effective viscosity depends on the drag coefficient and the density of the network. Dotted line indicates $(L/l_c-1)^2$ \textbf{b)}  The transition to viscous behavior occurs with a consistent timescale governed by the ratio of the elastic modulus, $G_0 \approx \mu/l_c$, to the viscosity $\eta_c$.  }
\end{figure}
...
\caption{\label{fig:active} Networks with free boundaries transiently contract in the presence of activity. \textbf{a)}  Example of an active network contracting. Note that the contraction halts between 100 s and 150 s. Network parameters: $L=5\: \mu m$, $l_c=0.3\: \mu m$, $\xi=100\: nN\cdot s$, $\upsilon=0.1\: nN$.  \textbf{b)} Traces of total network strain and internal strain of filaments.   \textbf{c)} The network's ability to deform requires asymmetric filament compliance.  Total network strain also increases with the applied myosin force $\upsilon$.}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=\hsize]{figures/figure4b}
\caption{\label{fig:active} Transient stress generation occurs with a characteristic timescale.  \textbf{a)} Example of an active network with fixed boundaries generating internal stresses but not deforming. Network parameters: $L=5\: \mu m$, $l_c=0.3\: \mu m$, $\xi=100\: nN\cdot s$, $\upsilon=0.1\: nN$.  \textbf{b)} Traces of total network stress and internal stress of filaments.   Internal stress buildup is only transient, and at long times there is an internal balance of extensional and compressive stresses \textbf{c}. Measurement and prediction of the timescale of the maximum stress. }
\end{figure}
...
\caption{\label{fig:recycle}  Filament recycling allows for steady state stresses to persist. \textbf{a)}  Example simulations of networks with varying timescales of recycling. Note that under the longer recycling time conditions the filaments are able to reach higher strain states than when the recycling time is shorter.  Network parameters: $L=5\: \mu m$, $l_c=0.3\: \mu m$, $\xi=100\: nN\cdot s$, $\upsilon=1\: nN$.  \textbf{b)} Traces of net stress for with different recycling timescales; shorter recycling times prevent the stress from relaxing entirely. However, very short recycling times prevent any stress from being built up in the first place. }
\end{figure}
...
\caption{\label{fig:recycle}  Filament recycling sets viscosity and steady state stress. \textbf{a)}  Normalized effective viscosities as a function of normalized recycling time.  When the recycling timescale is significantly less than the passive relaxation timescale discussed in Figure \ref{fig:passive_form}, the viscosity of the network becomes dependent on recycling time.  \textbf{b)} Normalized steady state stress as a function of normalized recycling time.  The steady state stress is set by the timescale at which the   }
\end{figure}
