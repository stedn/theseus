...
Taken together, these effects would suggest a simple predicted flow for a given network setup.  Assuming that the network will behave as a purely viscous material at long times, the strain rate will behave as $\dot{\gamma}=\sigma/\eta$. Due to the different dependences of both effective viscosity and active stress on filament recycling in different regimes, there will be a distinct dependence of flow rate (strain rate) on filament recycling as well.  To summarize the predicted behaviors we introduce Figure \ref{fig:flow_theo}. 
