...
%DIF DEL active_tosubmit.tex            Tue Jan 17 13:44:36 2017
%DIF ADD active_revision_tosubmit.tex   Sat Jun  3 20:29:33 2017
...
\DIFaddbegin \DIFadd{Margaret Gardel\textsuperscript{2},
}\DIFaddend Edwin M Munro\textsuperscript{3,*}
\\
\bigskip
...
We assume (Fig. \ref{fig:model_overview}A) that local deformation of filament i gives rise to \DIFdelbegin \DIFdel{an elastic force}\DIFdelend \DIFaddbegin \DIFadd{equal and opposite elastic restoring forces on endpoints i and i+1}\DIFaddend :

\begin{equation}
\DIFdelbegin %DIFDELCMD < %DIFDELCMD < \label{eqn:spring}%%%
%DIFDELCMD < %%%
\DIFdel{\mathbf{F^{\mu}_{i,i+1}} }\DIFdelend \DIFaddbegin \label{eqn:internal}
\DIFadd{\mathbf{F^{elas}_i} }\DIFaddend = \DIFdelbegin \DIFdel{\mu \gamma_{i}  \mathbf{\hat{u_i}}
}\DIFdelend \DIFaddbegin \DIFadd{\mathbf{F^{\mu}_{i}} 
\text{,} 
}\qquad
\DIFadd{\mathbf{F^{elas}_{i+1}} = \mathbf{-F^{\mu}_{i}}
...
%DIFDELCMD < %%%
\begin{displaymath}
\DIFdel{%DIFDELCMD < \label{eqn:internal}%%%
...
%DIFDELCMD < %%%
\DIFdel{In the limit of highly rigid cross-links and flexible filaments, our model approaches the pure semi-flexible filament models of \mbox{%DIFAUXCMD
\cite{theo_hlm,theo_hlm2}}%DIFAUXCMD
. In the opposite limit (nearly rigid filaments and highly flexible cross links), our model approaches that of \mbox{%DIFAUXCMD
\cite{theo_crosslinknonlinear} }%DIFAUXCMD
...
\label{exp_drag}
Previous models \DIFdelbegin \DIFdel{represent }\DIFdelend \DIFaddbegin \DIFadd{have represented }\DIFaddend cross-linkers as elastic connections between pairs of points on neighboring filaments that \DIFdelbegin \DIFdel{appear and disappear }\DIFdelend \DIFaddbegin \DIFadd{bind and unbind }\DIFaddend with either fixed or force-dependent probabilities \cite{model_taeyoon,theo_crosslinknonlinear}.  Here, we introduce a coarse-grained representation of crosslink dynamics by introducing an effective drag force that couples every pair of overlapping filaments, and which represents a molecular friction arising from the time-averaged contributions of many individual transient crosslinks (Fig. \ref{fig:model_overview}B). This \DIFdelbegin \DIFdel{coarse-grained approximation has been shown to be adequate in the case of }\DIFdelend \DIFaddbegin \DIFadd{is a reasonable approximation if all pairs of overlapping filaments have equal access to a non-limiting pool of cross links, and if the rate at which filaments moved past one another is slow relative to the unbinding rate of individual crosslinks \mbox{%DIFAUXCMD
\cite{howard2001mechanics}}%DIFAUXCMD
. This coarse-grained approach has been used to model frictional forces arising from }\DIFaddend ionic cross-linking of actin \DIFaddbegin \DIFadd{filaments }{\em \DIFadd{in vitro}} \DIFaddend \cite{mol_fric,theo_hydroish2}, and \DIFdelbegin \DIFdel{has been used to justify }\DIFdelend simple force-velocity \DIFdelbegin \DIFdel{curves for myosin bound filaments in other contexts \mbox{%DIFAUXCMD
\cite{theo_frictionShila}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{relationships for systems of cytoskeletal filaments and cross-linking motors \mbox{%DIFAUXCMD
\cite{theo_frictionShila, Schoenberg1985, tawada1991, leibler1993}}%DIFAUXCMD
}\DIFaddend .

To implement coupling through effective drag, for any pair of overlapping filaments \DIFdelbegin \DIFdel{j and k}\DIFdelend \DIFaddbegin \DIFadd{i and j}\DIFaddend , we write the drag force on filament \DIFdelbegin \DIFdel{j }\DIFdelend \DIFaddbegin \DIFadd{i }\DIFaddend as:

\begin{equation}
\label{eqn:drag force}
\DIFdelbegin \DIFdel{\mathbf{F^{\xi}_{j,k}} }\DIFdelend \DIFaddbegin \DIFadd{\mathbf{F^{\xi}_{i,j}} }\DIFaddend = -\xi  (\DIFdelbegin \DIFdel{\mathbf{v_{j}}}\DIFdelend \DIFaddbegin \DIFadd{\mathbf{v_{i}}}\DIFaddend -\DIFdelbegin \DIFdel{\mathbf{v_{k}}}\DIFdelend \DIFaddbegin \DIFadd{\mathbf{v_{j}}}\DIFaddend ) 
\end{equation}

where $\xi$ is the drag coefficient and \DIFdelbegin \DIFdel{$\mathbf{v_{j}}$, $\mathbf{v_{k}}$ }\DIFdelend \DIFaddbegin \DIFadd{$\mathbf{v_{i}}$, $\mathbf{v_{j}}$ }\DIFaddend are the average velocities of filaments \DIFdelbegin \DIFdel{j and k}\DIFdelend \DIFaddbegin \DIFadd{i and j}\DIFaddend . We apportion this drag force to the two endpoints ( \DIFdelbegin \DIFdel{j, j}\DIFdelend \DIFaddbegin \DIFadd{i, i}\DIFaddend +1) of filament \DIFdelbegin \DIFdel{j }\DIFdelend \DIFaddbegin \DIFadd{i }\DIFaddend as follows: If \DIFdelbegin \DIFdel{$\mathbf{x_{j,k}}$ }\DIFdelend \DIFaddbegin \DIFadd{$\mathbf{x_{i,j}}$ }\DIFaddend is the position of the filament overlap, then we \DIFdelbegin \DIFdel{assign $(1 - \mathbf{\lambda_{j,k}}) \mathbf{F^{\xi}_{j,k}}$ to endpoint j and $\mathbf{\lambda_{j,k}} \mathbf{F^{\xi}_{j,k}}$ to endpoint j}\DIFdelend \DIFaddbegin \DIFadd{define $\mathbf{\lambda_{i,j}} = |\mathbf{x_{i,j}}-\mathbf{x_i}|/|\mathbf{x_{i+1}}-\mathbf{x_i}|$ to be the fractional position of the overlap point along filament i, and we assign $(1 - \mathbf{\lambda_{i,j}}) \mathbf{F^{\xi}_{i,j}}$ to endpoint i and $\mathbf{\lambda_{i,j}} \mathbf{F^{\xi}_{i,j}}$ to endpoint i}\DIFaddend +\DIFdelbegin \DIFdel{1, where $\mathbf{\lambda_{j,k}} = |\mathbf{x_{j,k}}-\mathbf{x_j}|/|\mathbf{x_{j+1}}-\mathbf{x_j}|$.
}\DIFdelend \DIFaddbegin \DIFadd{1.
}\DIFaddend 

The total crosslink coupling \DIFdelbegin \DIFdel{force on endpoint i}\DIFdelend \DIFaddbegin \DIFadd{forces on endpoints i and i+1 }\DIFaddend due to overlaps along filament i \DIFdelbegin \DIFdel{and i-1 }\DIFdelend can then be written:

\begin{equation}
\label{eqn: total drag couple}
\DIFdelbegin \DIFdel{\mathbf{F^{xl}_{i}} = \sum_j (1 - \mathbf{\lambda_{i,j}}) \mathbf{F^{\xi}_{i,j}} + \sum_k \mathbf{\lambda_{i-1,k}} \mathbf{F^{\xi}_{i-1,k}}
...
where the sums are taken over all filaments j \DIFdelbegin \DIFdel{and k }\DIFdelend that overlap with filaments i\DIFdelbegin \DIFdel{and i-1 respectively}\DIFdelend .  
...
To add motor activity at the point of overlap between two filaments \DIFdelbegin \DIFdel{j and k }\DIFdelend \DIFaddbegin \DIFadd{i and j }\DIFaddend ; for each filament in the pair, we impose an additional force of magnitude $\upsilon$, directed towards its (-) end (Fig. \ref{fig:model_overview}C):
...
and we impose an equal and opposite force on its overlapping partner.  We distribute these forces to filament endpoints as described above for crosslink coupling forces.  Thus, the total force on \DIFdelbegin \DIFdel{endpoint i }\DIFdelend \DIFaddbegin \DIFadd{endpoints i and i+1 }\DIFaddend due to motor activity \DIFaddbegin \DIFadd{on overlap points between filaments i and j }\DIFaddend can be written as:

\begin{equation}
\label{eqn:active}
\DIFdelbegin \DIFdel{\mathbf{F^{motor}_{i}} = \upsilon \sum_j (1 - \mathbf{\lambda_{i,j}}) }%DIFDELCMD < \left (%%%
\DIFdel{\mathbf{\hat{u_{i}}} - \mathbf{\hat{u_j}} }%DIFDELCMD < \right ) %%%
\DIFdel{q_{i,j}
                        +  \upsilon \sum_k (\mathbf{\lambda_{i-1,k}}) }%DIFDELCMD < \left (%%%
\DIFdel{\mathbf{\hat{u_{i-1}}} - \mathbf{\hat{u_k}} }%DIFDELCMD < \right ) %%%
\DIFdel{q_{i-1,k} 
}\DIFdelend \DIFaddbegin \begin{aligned}
...
where j \DIFdelbegin \DIFdel{and k enumerate }\DIFdelend \DIFaddbegin \DIFadd{enumerates }\DIFaddend over all filaments \DIFaddbegin \DIFadd{j }\DIFaddend that overlap with \DIFdelbegin \DIFdel{filaments i}\DIFdelend \DIFaddbegin \DIFadd{filament i, }\DIFaddend and \DIFdelbegin \DIFdel{i-1 respectively, and $q_{j,k}$ }\DIFdelend \DIFaddbegin \DIFadd{$q_{i,j}$ }\DIFaddend equals 0 or 1 depending on whether there is an ``active'' motor at this location. To model dispersion of motor activity, we set $q_{i,j}=1$  on a randomly selected subset of filament overlaps, such that $\bar{q}=\phi$, where $\bar{q}$ indicates the mean of $q$ (Fig. \ref{fig:model_overview}C).
...
where $\mathbf{x}$ is a vector of filament endpoint positions, $\mathbf{\dot{x}}$ the endpoint velocities, $\mathbf{A }$ is a matrix with constant coefficients that represent crosslink coupling forces between overlapping filaments, and $\mathbf{f(x)}$ represents the active (motor) and elastic forces on filament endpoints. We smoothed all filament interactions, force fields, and constraints linearly over small regions such that the equations contained no sharp discontinuities. We numerically integrate this system of equations to find the time evolution of the positions of all filament endpoints. We generate a network of filaments with random positions and orientations as described above within a domain of size $D_x$ by $D_y$.  For all simulations, we imposed periodic boundaries in the y-dimension. To impose an extensional \DIFdelbegin \DIFdel{stress}\DIFdelend \DIFaddbegin \DIFadd{force per unit length ( 2D stress) on the network}\DIFaddend , we constrained all filament endpoints within a fixed distance $0.05\cdot D_x$ from the left edge of the domain to be non-moving, then we imposed a rightwards force on all endpoints within a distance $0.05\cdot D_x$ from the right edge of the domain\DIFaddbegin \DIFadd{, such that the force per unit length of boundary equals the desired stress value}\DIFaddend . To simulate free contraction, we removed all constraints at domain boundaries; to assess buildup and maintenance of contractile stress under isometric conditions, we used periodic boundary conditions in both $x$ and $y$ dimensions.

\DIFdelbegin \DIFdel{We measured the local velocity of the network at different positions along the axis of deformation as the mean velocity of all filaments intersecting that position; we }\DIFdelend \DIFaddbegin \subsection*{\DIFadd{Measuring stress, strain, and strain rate}}
\DIFadd{In our 2D model, The analogue of stress is a force per unit length, which we refer to simply as a stress. We }\DIFaddend measured the internal network stress at each axial position by summing the axial \DIFaddbegin \DIFadd{(x) }\DIFaddend component of the tensions on all filaments intersecting that position, and dividing by network height \DIFdelbegin \DIFdel{; finally, we measured }\DIFdelend \DIFaddbegin \DIFadd{$D_y$.  We measured the strain on an individual filament as the change in filament length, divided by its equilibrium length.  Then we averaged this measurement over all filaments in a network to obtain an average filament strain.  To measure the average }\DIFaddend network strain rate\DIFdelbegin \DIFdel{as the average of all filament velocities divided by their positions}\DIFdelend \DIFaddbegin \DIFadd{, we first measured the mean velocity v(X) at position X (relative to the network boundary at x = 0) to be the average velocities of all filaments intersecting that position.  In the cases where we measure network strain or strain rates, we observed an approximately linear dependence of v(X) on X; hence the strain rate is approximately uniform across the network (Fig. \ref{fig:passive_ex}B and }\nameref{fig:combo_prof}\DIFadd{). Accordingly, for each filament, we took $\frac{1}{X} \frac{dX}{dt}$ to be an estimate of the strain rate on the network between x = 0 and x = X.  We averaged this estimate over all filaments in a domain to get an average strain rate.  Finally, to estimate the cumulative network strain at a given time T in the simulation, we integrated the strain rate with respect to time for t = 0 to T}\DIFaddend . 
...
As shown in Fig. \ref{fig:passive_form}B, our simulations agree well with this prediction for a large range of sampled network parameters. Finally, for many linear viscoelastic materials, the ratio of effective viscosity to the elastic modulus $\eta_c/G_0$ sets the timescale for transition from elastic to viscous behavior\cite{mccrum1997principles}. Combining our approximations for $G_0$ and $\eta_c$, we predict a transition time, $\tau_c \approx L^2\xi/l_c\mu$. Measuring the time at which the strain rate became nearly constant (i.e. $\gamma \sim t^n$ with $n>0.8$) yields an estimate of $\tau_c$ that agrees well with this prediction over the entire range of sampled parameters (Fig. \ref{fig:passive_form}C).  Thus the passive response of filament networks with frictional cross link drag is well-described on short (viscoelastic) to intermediate (viscous) timescales by an elastic modulus $G_0$, an effective viscosity $\eta_c$, and a transition timescale $\tau_c$, with well-defined dependencies on network parameters. \DIFdelbegin \DIFdel{However, without filament turnover, strain thinning and network tearing limits the extent of viscous deformation to small strains}\DIFdelend \DIFaddbegin \DIFadd{On longer timescales, the cumulative decrease in filament density leads to material failure}\DIFaddend .

% Passive Recycling
\paragraph{Filament turnover allows sustained large-scale viscous flow and defines two distinct flow regimes.} 


To characterize how filament turnover shapes the passive network response to an applied force, we introduced a simple form of turnover in which entire filaments disappear at a rate $k_{diss}\rho$, where $\rho$ is the filament density, and new unstrained filaments appear with a fixed rate per unit area, \DIFdelbegin \DIFdel{$k_{app}$. In a non-deforming network}\DIFdelend \DIFaddbegin \DIFadd{$k_{ass}$. Absent deformation}\DIFaddend ,  filament density will equilibrate to a steady state value, $\rho_0 = k_{ass}/k_{diss}$, with time constant $\tau_r = 1/k_{diss}$.  However, in networks deforming under extensional stress, \DIFdelbegin \DIFdel{the }\DIFdelend density will be \DIFdelbegin \DIFdel{set by a competition between strain thinning and density equilibration via turnover}\DIFdelend \DIFaddbegin \DIFadd{shaped by both turnover and network extension, and changes in density will feed back on deformation through the density-dependence of effective viscosity (Fig. \ref{fig:passive_form}B)}\DIFaddend . 

\DIFdelbegin \DIFdel{We }\DIFdelend \DIFaddbegin \DIFadd{To examine the consequences of these effects, we }\DIFaddend simulated a uniaxial stress test for different values of $\tau_r$, while holding \DIFaddbegin \DIFadd{$\rho_0$ and }\DIFaddend all other parameters fixed (Fig. \ref{fig:passive_rec}A-C). For large $\tau_r$, \DIFdelbegin \DIFdel{as described above, the network undergoes strain thinning and ultimately tears.  Decreasing }\DIFdelend \DIFaddbegin \DIFadd{network extension was accompanied by a continuous decrease in filament density, leading ultimately to loss of connectivity and material failure (}\nameref{fig:thinning}\DIFadd{).  For lower values of }\DIFaddend $\tau_r$\DIFdelbegin \DIFdel{increases the rate at which the network equilibrates towards }\DIFdelend \DIFaddbegin \DIFadd{, the network approached }\DIFaddend a steady state \DIFdelbegin \DIFdel{density $\rho_0$.  However, it also increases the rate of deformation and thus the rate of strain thinning }\DIFdelend \DIFaddbegin \DIFadd{characterized by continuous extension at a constant density and strain rate }\DIFaddend (Fig. \ref{fig:passive_rec}B\DIFdelbegin \DIFdel{).  We found that the former }\DIFdelend \DIFaddbegin \DIFadd{, }\nameref{fig:thinning}\DIFadd{).  Analysis of a simple coarse-grained model (}\nameref{S1_Text} \DIFadd{A.3) revealed how this behavior emerges from a competition between density equilibration via turnover and the decrease in density, and thus effective viscosity, during extension. For $\tau_r$ greater than a critical lifetime $\tau_{crit}$, the latter }\DIFaddend effect dominates, \DIFdelbegin \DIFdel{such that below a critical value $\tau_r = \tau_{crit}$, the network can achieve a }\DIFdelend \DIFaddbegin \DIFadd{leading to a runaway decrease in filament density, loss of connectivity and material failure. For $\tau_r < \tau_{crit}$, the coarse-grained model predicts the existence of a dynamically stable }\DIFaddend steady state characterized by \DIFdelbegin \DIFdel{a fixed density and a constant }\DIFdelend \DIFaddbegin \DIFadd{continuous extension at constant density and }\DIFaddend strain rate (\DIFdelbegin %DIFDELCMD < \nameref{fig:thinning}%%%
\DIFdel{).  Simple calculations (}\DIFdelend \nameref{S1_Text} A.3)\DIFdelbegin \DIFdel{show that the critical value of $\tau_r$ is approximately:
}%DIFDELCMD < 

%DIFDELCMD < %%%
\begin{displaymath}
\DIFdel{%DIFDELCMD < \label{eqn:syst3}%%%
...
%DIFDELCMD < %%%
\DIFdel{where  $\sigma$ is the applied stress, $L/l_c$ the linear cross link density, and $\xi$ is the effective crosslink drag}\DIFdelend \DIFaddbegin \DIFadd{, as observed in our simulations (}\nameref{fig:active_supp}\DIFadd{)}\DIFaddend . 
...
To confirm this prediction, we simulated the simple scenario described above for a range of values of $\tau_r$, with all other parameter values initially fixed \DIFaddbegin \DIFadd{at their reference values (See Table \ref{table:para})}\DIFaddend . 
...
\DIFaddbegin \DIFadd{For the reference values of model parameters, the steady state strain rates ($\approx 2x10^-4/sec$) were somewhat lower than, but comparable to those measured during steady state flows in }\textit{\DIFadd{C. elegans}} \DIFadd{($\approx 1.3x10^-3/sec$) \mbox{%DIFAUXCMD
\cite{sailer2015}}%DIFAUXCMD
.
}\DIFaddend 
...
{\bf  Fast viscoelastic response to extensional stress.}  Plots of normalized \DIFaddbegin \DIFadd{cumulative }\DIFaddend strain vs time during the elastic phase of deformation in passive networks under extensional stress.  Measured strain is normalized by the equilibrium strain predicted for a network of elastic filaments without crosslink slip $\gamma_{eq} = \sigma/G_0 = \sigma/(2\mu/l_c)$.  


\paragraph*{S2 Fig.}
\label{fig:thinning}
{\bf  Filament turnover \DIFdelbegin \DIFdel{rescues strain thinning}\DIFdelend \DIFaddbegin \DIFadd{limits density decrease under extensional strain and allows continuous flow without material failure}\DIFaddend .} \textbf{A)} Plots of \DIFaddbegin \DIFadd{cumulative }\DIFaddend strain vs time for different turnover times (see inset in (b)). Note the increase in strain rates with decreasing turnover time. \textbf{B)} Plots of filament \DIFdelbegin \DIFdel{density vs strain }\DIFdelend \DIFaddbegin \DIFadd{length density vs time }\DIFaddend for different turnover times $\tau_r$.  For \DIFaddbegin \DIFadd{long to }\DIFaddend intermediate $\tau_r$, simulations predict \DIFdelbegin \DIFdel{progressive strain thinning, but at a lower rate than in the complete absence of recycling. For higher }\DIFdelend \DIFaddbegin \DIFadd{an $\approx$ linear decrease in length density with time, at a rate that decreases with decreasing $\tau_r$, leading ultimately to loss of connectivity and material failure. For lower }\DIFaddend $\tau_r$, \DIFaddbegin \DIFadd{length }\DIFaddend densities approach steady state values at longer times. \DIFaddbegin \DIFadd{These results match the predictions of the corase grained analysis in Appendix section A.3.
}\DIFaddend 

\paragraph*{S3 Fig.}
\label{fig:active_supp}
{\bf  Mechanical properties of active networks.}  \textbf{A)}  Time for freely contracting networks to reach maximum \DIFaddbegin \DIFadd{cumulative }\DIFaddend strain, $\tau_s$, scales with $L\xi/\upsilon$.  \textbf{B)} Free contraction requires asymmetric filament compliance, and total \DIFaddbegin \DIFadd{cumulative }\DIFaddend network strain increases with the applied myosin force $\upsilon$. Note that the maximum contraction approaches an asymptotic limit as the stiffness asymmetry approaches a ratio of $\sim 100$.   \textbf{C)}  Maximum stress achieved during isometric contraction, $\sigma_m$, scales approximately with $\sqrt{\mu_e\upsilon}/l_c$.  \textbf{D)} Time to reach max stress during isometric contraction scales approximately with $L\xi/\sqrt{\mu_e\upsilon}$. Scalings for $\tau_s$, $\sigma_m$ and $\tau_m$ were determined empirically by trial and error, guided by dimensional analysis.  

\paragraph*{S4 Fig.}
\label{fig:active_tear}
{\bf  Filament turnover prevents \DIFaddbegin \DIFadd{loss of connectivity and local }\DIFaddend tearing of active networks.}  \DIFdelbegin \DIFdel{Plots of normalized strain vs time during the elastic phase of deformation in passive networks under extensional stress.  Measured strain is normalized by the equilibrium strain predicted for a network of elastic filaments without crosslink slip $\gamma_{eq} = \sigma/G_0 = \sigma/(2\mu/l_c)$}\DIFdelend \DIFaddbegin \textbf{\DIFadd{A)}}  \DIFadd{An active network undergoing large scale deformations due to active filament rearrangements.  }\textbf{\DIFadd{B)}}  \DIFadd{The same network as in (A) but with a shorter filament turnover time.  }\textbf{\DIFadd{C)}}  \DIFadd{Plots of internal stress vs time for the network in (A).  }\textbf{\DIFadd{D)}}  \DIFadd{Plots of internal stress vs time for the network in (B)}\DIFaddend .  
...
{\bf  Dynamics of steady state flow.}  Plots of stress and \DIFaddbegin \DIFadd{cumulative }\DIFaddend strain vs position for networks in which motor activity is limited to the right-half domain and filament turnover time is either  \textbf{A)} $\tau_r = 10000$ or  \textbf{B)} $\tau_r = 10 s$.  Blue indicates \DIFaddbegin \DIFadd{average filament }\DIFaddend velocity while orange represents \DIFdelbegin \DIFdel{total }\DIFdelend \DIFaddbegin \DIFadd{average network }\DIFaddend stress, measured as described in the main text. 
