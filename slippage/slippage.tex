...
We recognize the constant of proportionality between stress and strain rate as a viscosity.  Therefore, our approximation for the effective viscosity, $\eta_{eft}$, at steady state creep in this low strain limit is
...
\subsection{Effect from Large Strains}
...
\subsubsection{Filament Alignment And Network Tearing}
...
At this point even the mean filament stretch becomes anisotropic $\langle \delta L / L \rangle \neq \gamma_{xy}sin(\theta)cos(\theta)$, with a larger number of filaments undergoing extension than compression.  This results in a tighter peak around $-pi/4$ and a broader one at $pi/4$.
